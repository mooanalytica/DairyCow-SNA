...
class Kpt2dSviewRgbImgTopDownDataset(Dataset, metaclass=ABCMeta):
    ...
    def _xywh2cs(self, x, y, w, h, padding=1.25):
        ...
        aspect_ratio = self.ann_info['image_size'][0] / self.ann_info[
            'image_size'][1]
        center = np.array([x + w * 0.5, y + h * 0.5], dtype=float)
    ...
    def _get_normalize_factor(self, gts, *args, **kwargs):
        ...
        """
        return np.ones([gts.shape[0], 2], dtype=float)
    ...
    def _report_metric(self,
        ...
        if 'PCK' in metrics:
            parts, pck, _ = keypoint_pck_accuracy(outputs, gts, masks, pck_thr,
                                              threshold_bbox)
            info_str.append(('PCK', pck))
            info_str.append(('PCK_parts', parts))
