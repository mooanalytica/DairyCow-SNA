...
def get_data_sample():
    ...
    _joints_2d = data['part'][:, H36M_JOINT_IDX].astype(float)
    _joints_3d = data['S'][:, H36M_JOINT_IDX].astype(float)
    ...
    ann_info['joint_weights'] = np.full(17, 1.0, dtype=float)
...
def test_joint_transforms():
    ...
    mean = np.random.rand(16, 3).astype(float)
    std = np.random.rand(16, 3).astype(float) + 1e-6
    ...
    center = np.array(
        ...
        dtype=float)
    scale = np.array(0.5 * results['image_width'], dtype=float)
...
def test_3d_heatmap_generation():
    ann_info = dict(
        ...
        num_joints=17,
        joint_weights=np.ones((17, 1), dtype=float),
