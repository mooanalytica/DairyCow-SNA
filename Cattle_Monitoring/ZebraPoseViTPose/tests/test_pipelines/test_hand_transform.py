...
def get_sample_data():
    ...
    results = {
        ...
        'center': np.asarray([200, 200], dtype=float),
        'scale': 1.0,
        'rotation': 0,
        'joints_3d': np.zeros([42, 3], dtype=float),
        'joints_3d_visible': np.ones([42, 3], dtype=float),
        'hand_type': np.asarray([1, 0], dtype=float),
        'hand_type_valid': 1,
        'rel_root_depth': 50.0,
        'rel_root_valid': 1,
        'ann_info': ann_info
