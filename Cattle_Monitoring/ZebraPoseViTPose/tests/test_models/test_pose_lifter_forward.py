...
def _create_inputs(joint_num_in,
    ...
    meta_info = {
        'root_position': np.zeros((1, joint_channel_out), float),
        'root_position_index': 0,
        'target_mean': np.zeros((joint_num_out, joint_channel_out),
                                float),
        'target_std': np.ones((joint_num_out, joint_channel_out), float)
    }
    ...
    if semi:
        traj_target = np.zeros((batch_size, 1, joint_channel_out), float)
        unlabeled_pose_in = rng.rand(batch_size,
                                     joint_num_in * joint_channel_in, seq_len)
        unlabeled_target_2d = np.zeros(
            (batch_size, joint_num_out, joint_channel_in), dtype=float)
