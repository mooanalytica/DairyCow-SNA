...
def _collate_pose_sequence(pose_results, with_track_id=True, target_frame=-1):
    ...
    for idx in range(N):
        ...
        if not with_track_id:
            ...
            keypoints = np.zeros((T, K, C), dtype=float)
...
def inference_pose_lifter_model(model,
    ...
    if dataset_info is not None:
        ...
        if dataset == 'Body3DH36MDataset':
            ...
            bbox_center = np.array([[528, 427]], dtype=float)
            bbox_scale = 400
        else:
    ...
    for seq in pose_sequences_2d:
        pose_2d = seq['keypoints'].astype(float)
        T, K, C = pose_2d.shape
        ...
        if C > 2:
            ...
            input_2d_visible = np.ones((T, K, 1), dtype=float)
        ...
        target = np.zeros((K, 3), dtype=float)
        target_visible = np.ones((K, 1), dtype=float)
...
def inference_interhand_3d_model(model,
    ...
    for bbox in bboxes:
        ...
        data = {
            ...
            dataset,
            'joints_3d':
            np.zeros((cfg.data_cfg.num_joints, 3), dtype=float),
            'joints_3d_visible':
            np.zeros((cfg.data_cfg.num_joints, 3), dtype=float),
            'rotation':
            0,
...
def inference_mesh_model(model,
    ...
    for bbox in bboxes:
        ...
        data = {
            ...
            dataset,
            'joints_2d':
            np.zeros((cfg.data_cfg.num_joints, 2), dtype=float),
            'joints_2d_visible':
            np.zeros((cfg.data_cfg.num_joints, 1), dtype=float),
            'joints_3d':
            np.zeros((cfg.data_cfg.num_joints, 3), dtype=float),
            'joints_3d_visible':
            np.zeros((cfg.data_cfg.num_joints, 3), dtype=float),
            'pose':
            np.zeros(72, dtype=float),
            'beta':
