...
class SimpleCamera(SingleCameraBase):
    ...
    def __init__(self, param):
        ...
        self.param = {}
        # extrinsic param
        R = np.array(param['R'], dtype=float)
        T = np.array(param['T'], dtype=float)
        assert R.shape == (3, 3)
        assert T.shape == (3, 1)
        ...
        if 'k' in param and 'p' in param:
            ...
            self.param['k'] = np.array(param['k'], dtype=float).flatten()
            self.param['p'] = np.array(param['p'], dtype=float).flatten()
