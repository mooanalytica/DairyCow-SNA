...
class Interhand3DHead(nn.Module):
    ...
    def decode(self, img_metas, output, **kwargs):
        ...
        heatmap3d_depth_bound = np.ones(batch_size, dtype=float)
        root_depth_bound = np.ones(batch_size, dtype=float)
        center = np.zeros((batch_size, 2), dtype=float)
        scale = np.zeros((batch_size, 2), dtype=float)
        image_paths = []
        score = np.ones(batch_size, dtype=float)
        ...
        all_boxes = np.zeros((batch_size, 6), dtype=float)
