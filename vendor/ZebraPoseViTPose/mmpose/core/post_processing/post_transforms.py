...
def _get_3rd_point(a, b):
    """Given two 2D points a,b (shape (2,)), return a third point that forms
    an oriented right triangle — used by MMPose to define an affine frame."""
    a = np.asarray(a, dtype=np.float32)
    b = np.asarray(b, dtype=np.float32)
    direct = a - b
    # rotate 90° to get a perpendicular vector
    return b + np.array([-direct[1], direct[0]], dtype=np.float32)

def rotate_point(pt, rot_rad):
    """Rotate a 2D point/vector by rot_rad (radians)."""
...
def get_affine_transform(center,
                         ...
                         shift=(0., 0.),
                         inv=False):
    """Get the 2×3 affine transform matrix mapping src↔dst.

    Args:
        center (array-like, 2): bbox center (x, y).
        scale (array-like, 2): bbox scale wrt [width, height], in 'pixel_std' units.
        rot (float): rotation angle in degrees.
        output_size (array-like, 2): destination (width, height).
        shift (tuple of float, 2): relative shift wrt width/height (0–1).
        inv (bool): if True, return inverse transform (dst→src).

    Returns:
        np.ndarray (2,3) float32: affine matrix.
    """
    # --- sanitize inputs & dtypes (OpenCV expects float32) ---
    center = np.asarray(center, dtype=np.float32)
    scale  = np.asarray(scale,  dtype=np.float32)
    shift  = np.asarray(shift,  dtype=np.float32)
    output_size = np.asarray(output_size, dtype=np.float32)

    assert center.shape == (2,),  "center must be length-2 (x, y)"
    assert scale.shape  == (2,),  "scale must be length-2 [sx, sy]"
    assert output_size.shape == (2,), "output_size must be (w, h)"
    assert shift.shape  == (2,),  "shift must be length-2"

    # pixel_std used by MMPose is 200
    scale_tmp = scale * 200.0
    ...
    # direction vectors
    # same as MMPose: start from vector [0, -0.5*src_w] and rotate by rot_rad
    src_dir = rotate_point([0., -0.5 * src_w], rot_rad)
    dst_dir = np.array([0., -0.5 * dst_w], dtype=np.float32)

    # --- build src triangle (3×2 float32) ---
    src = np.zeros((3, 2), dtype=np.float32)
    src[0] = center + scale_tmp * shift
    src[1] = center + src_dir + scale_tmp * shift
    src[2] = _get_3rd_point(src[0], src[1])

    # --- build dst triangle (3×2 float32) ---
    dst_center = np.array([0.5 * dst_w, 0.5 * dst_h], dtype=np.float32)
    dst = np.zeros((3, 2), dtype=np.float32)
    dst[0] = dst_center
...
def _get_3rd_point(a, b):
    ...
    assert len(b) == 2
    direction = a - b
    third_pt = b + np.array([-direction[1], direction[0]], dtype=float)
