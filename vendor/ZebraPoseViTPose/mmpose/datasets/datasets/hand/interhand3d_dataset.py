...
class InterHand3DDataset(Kpt3dSviewRgbImgTopDownDataset):
    ...
    def _get_db(self):
        ...
        for img_id in self.img_ids:
            ...
            camera_pos = np.array(
                cameras[capture_id]['campos'][camera_name], dtype=float)
            camera_rot = np.array(
                cameras[capture_id]['camrot'][camera_name], dtype=float)
            focal = np.array(
                cameras[capture_id]['focal'][camera_name], dtype=float)
            principal_pt = np.array(
                cameras[capture_id]['princpt'][camera_name], dtype=float)
            joint_world = np.array(
                joints[capture_id][frame_idx]['world_coord'], dtype=float)
            ...
            if self.use_gt_root_depth:
                ...
                bbox = np.array(rootnet_ann_data['bbox'], dtype=float)
    ...
    def _report_metric(self, res_file, metrics):
            ...
            if 'MRRPE' in metrics:
                if item['hand_type'].all() and item['joints_3d_visible'][
                    ...
                    pred_left_root_img = np.array(
                        pred['keypoints'][41], dtype=float)[None, :]
                    ...
                    pred_right_root_img = np.array(
                        pred['keypoints'][20], dtype=float)[None, :]
        ...
        gts_rel_root = np.array(gts_rel_root, dtype=float)
        preds_rel_root = np.array(preds_rel_root, dtype=float)
        rel_root_masks = np.array(rel_root_masks, dtype=bool)[:, None]
        gts_joint_coord_cam = np.array(gts_joint_coord_cam, dtype=float)
        preds_joint_coord_cam = np.array(
            preds_joint_coord_cam, dtype=float)
