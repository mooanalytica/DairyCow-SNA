...
class Kpt3dSviewKpt2dDataset(Dataset, metaclass=ABCMeta):
    ...
    def load_annotations(self):
        ...
        if 'scale' in data:
            ...
            _scales = np.zeros(num_imgs, dtype=float)

        if 'center' in data:
            _centers = data['center'].astype(float)
        else:
            _centers = np.zeros((num_imgs, 2), dtype=float)

        # get 3D pose
        if 'S' in data.keys():
            _joints_3d = data['S'].astype(float)
        else:
            _joints_3d = np.zeros((num_imgs, num_joints, 4), dtype=float)

        # get 2D pose
        if 'part' in data.keys():
            _joints_2d = data['part'].astype(float)
        else:
            _joints_2d = np.zeros((num_imgs, num_joints, 3), dtype=float)
