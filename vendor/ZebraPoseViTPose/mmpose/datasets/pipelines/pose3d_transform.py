...
class Generate3DHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        target = np.zeros([num_joints, D, H, W], dtype=float)

        target_weight = joints_3d_visible[:, 0].astype(float)
        ...
        x = y = z = np.arange(2 * tmp_size + 1, dtype=float) - tmp_size
        zz, yy, xx = np.meshgrid(z, y, x)
        xx = xx[None, ...].astype(float)
        yy = yy[None, ...].astype(float)
        zz = zz[None, ...].astype(float)
