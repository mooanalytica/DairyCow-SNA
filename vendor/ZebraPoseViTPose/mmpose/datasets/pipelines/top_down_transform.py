...
class TopDownHalfBodyTransform:
    ...
    def half_body_transform(cfg, joints_3d, joints_3d_visible):
        ...
        selected_joints = np.array(selected_joints, dtype=float)
        ...
        scale = np.array([w / 200.0, h / 200.0], dtype=float)
        scale = scale * 1.5
        return center, scale
...
class TopDownGenerateTarget:
    ...
    def _msra_generate_target(self, cfg, joints_3d, joints_3d_visible, sigma):
        ...
        target_weight = np.zeros((num_joints, 1), dtype=float)
        target = np.zeros((num_joints, H, W), dtype=float)
    ...
    def _udp_generate_target(self, cfg, joints_3d, joints_3d_visible, factor,
        ...
        target_weight = np.ones((num_joints, 1), dtype=float)
    ...
    def __call__(self, results):
        ...
        if self.encoding == 'MSRA':
            if isinstance(self.sigma, list):
                ...
                target = np.empty(
                    ...
                    dtype=float)
                target_weight = np.empty((0, num_joints, 1), dtype=float)
            ...
            if isinstance(self.kernel, list):
                ...
                target = np.empty((0, num_joints, H, W), dtype=float)
                target_weight = np.empty((0, num_joints, 1), dtype=float)
        ...
        if results['ann_info'].get('max_num_joints', None) is not None:
            ...
            target_weight = np.concatenate([target_weight, np.zeros((padded_length, 1), dtype=float)], 0)
            target = np.concatenate([target, np.zeros((padded_length, H, W), dtype=float)], 0)
