...
class HeatmapGenerator:
    ...
    def __init__(self, output_size, num_joints, sigma=-1, use_udp=False):
        ...
        if use_udp:
            self.x = np.arange(0, size, 1, float)
            self.y = self.x[:, None]
        else:
...
class BottomUpRandomAffine:
    ...
    def _get_scale(self, image_size, resized_size):
        ...
        scale = np.array([w_pad, h_pad], dtype=float)
    ...
    def __call__(self, results):
        ...
        img_scale = np.array([width, height], dtype=float)
        ...
        if self.use_udp:
            for i, _output_size in enumerate(self.output_size):
                ...
                trans = get_warp_matrix(
                    ...
                    size_dst=np.array(
                        (_output_size[0], _output_size[1]), dtype=float) -
                    1.0,
                    size_target=scale)
                ...
                mask[i] = (mask[i] > 0.5).astype(float)
                joints[i][:, :, 0:2] = \
...
class BottomUpGenerateHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        for scale_id in range(results['ann_info']['num_scales']):
            ...
            target_list.append(heatmaps.astype(float))
...
class BottomUpGenerateTarget:
    ...
    def __call__(self, results):
        ...
        for scale_id in range(results['ann_info']['num_scales']):
            ...
            target_list.append(target_t.astype(float))
            mask_list[scale_id] = mask_list[scale_id].astype(float)
...
class BottomUpGeneratePAFTarget:
    ...
    def __call__(self, results):
        ...
        for scale_id in range(results['ann_info']['num_scales']):
            ...
            target_list.append(pafs.astype(float))
