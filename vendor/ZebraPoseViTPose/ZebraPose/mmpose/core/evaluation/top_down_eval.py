...
def _calc_distances(preds, targets, mask, normalize):
    ...
    distances = np.full((N, K), -1, dtype=float)
...
def _get_max_preds_3d(heatmaps):
    ...
    preds = np.zeros((N, K, 3), dtype=float)
    _idx = idx[..., 0]
    preds[..., 2] = _idx // (H * W)
    preds[..., 1] = (_idx // W) % H
    preds[..., 0] = _idx % W
...
def keypoint_epe(pred, gt, mask):
    ...
    distances = _calc_distances(
        pred, gt, mask,
        np.ones((pred.shape[0], pred.shape[2]), dtype=float))
    distance_valid = distances[distances != -1]
    return distance_valid.sum() / max(1, len(distance_valid))
...
def post_dark_udp(coords, batch_heatmaps, kernel=3):
    ...
    hessian = np.linalg.inv(hessian + np.finfo(float).eps * np.eye(2))
...
def keypoints_from_regression(regression_preds, center, scale, img_size):
    ...
    preds, maxvals = regression_preds, np.ones((N, K, 1), dtype=float)
