...
class TopdownHeatmapBaseHead(nn.Module):
    ...
    def decode(self, img_metas, output, **kwargs):
        ...
        c = np.zeros((batch_size, 2), dtype=float)
        s = np.zeros((batch_size, 2), dtype=float)
        image_paths = []
        ...
        preds, maxvals, heatmpas = keypoints_from_heatmaps(
            output,
            c,
            s,
        ...
        all_preds = np.zeros((batch_size, preds.shape[1], 3), dtype=float)
