...
def test_pose_pck_accuracy():
    output = np.zeros((1, 5, 64, 64), dtype=float)
    target = np.zeros((1, 5, 64, 64), dtype=float)
...
def test_keypoints_from_heatmaps():
    heatmaps = np.ones((1, 1, 64, 64), dtype=float)
    heatmaps[0, 0, 31, 31] = 2
    center = np.array([[127, 127]])
    scale = np.array([[64 / 200.0, 64 / 200.0]])

    udp_heatmaps = np.ones((32, 17, 64, 64), dtype=float)
...
def test_keypoint_pck_accuracy():
    ...
    thr = np.full((2, 2), 10, dtype=float)
    # first channel
    output[0, 0] = [10, 0]
    target[0, 0] = [10, 0]
    # second channel
...
def test_keypoints_from_heatmaps3d():
    heatmaps = np.ones((1, 1, 64, 64, 64), dtype=float)
