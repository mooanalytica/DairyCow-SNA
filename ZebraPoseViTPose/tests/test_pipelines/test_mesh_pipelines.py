...
def _box2cs(box, image_size):
    ...
    if w > aspect_ratio * h:
        ...
        w = h * aspect_ratio
    scale = np.array([w * 1.0 / 200.0, h * 1.0 / 200.0], dtype=float)
    scale = scale * 1.25
    return center, scale
...
def _load_test_data():
    ...
    ann_info['joint_weights'] = \
        np.ones(ann_info['num_joints'], dtype=float
    ...
    results['scale'] = np.array([scale, scale]).astype(float)
    results['center'] = ann_data['center'][index].astype(float)

    # Get gt 2D joints, if available
    if 'part' in ann_data.keys():
        keypoints = ann_data['part'][index].astype(float)
        results['joints_2d'] = keypoints[:, :2]
        results['joints_2d_visible'] = keypoints[:, -1][:, np.newaxis]
    else:
        results['joints_2d'] = np.zeros((24, 2), dtype=float)
        results['joints_2d_visible'] = np.zeros((24, 1), dtype=float)

    # Get gt 3D joints, if available
    if 'S' in ann_data.keys():
        joints_3d = ann_data['S'][index].astype(float)
        results['joints_3d'] = joints_3d[:, :3]
        results['joints_3d_visible'] = joints_3d[:, -1][:, np.newaxis]
    else:
        results['joints_3d'] = np.zeros((24, 3), dtype=float)
        results['joints_3d_visible'] = np.zeros((24, 1), dtype=float)

    # Get gt SMPL parameters, if available
    if 'pose' in ann_data.keys() and 'shape' in ann_data.keys():
        results['pose'] = ann_data['pose'][index].astype(float)
        results['beta'] = ann_data['shape'][index].astype(float)
        results['has_smpl'] = 1
    else:
        results['pose'] = np.zeros(72, dtype=float)
        results['beta'] = np.zeros(10, dtype=float)
        results['has_smpl'] = 0
