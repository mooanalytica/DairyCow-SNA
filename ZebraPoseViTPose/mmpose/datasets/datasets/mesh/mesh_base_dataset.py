...
class MeshBaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.ann_info['joint_weights'] = np.ones([24, 1], dtype=float)
    ...
    def _get_db(self, ann_file):
        ...
        _scales = data['scale'].astype(float)
        _centers = data['center'].astype(float)
        dataset_len = len(_imgnames)

        # Get 2D keypoints
        if 'part' in data.keys():
            _keypoints = data['part'].astype(float)
        else:
            _keypoints = np.zeros((dataset_len, 24, 3), dtype=float)

        # Get gt 3D joints, if available
        if 'S' in data.keys():
            _joints_3d = data['S'].astype(float)
        else:
            _joints_3d = np.zeros((dataset_len, 24, 4), dtype=float)

        # Get gt SMPL parameters, if available
        if 'pose' in data.keys() and 'shape' in data.keys():
            _poses = data['pose'].astype(float)
            _betas = data['shape'].astype(float)
            has_smpl = 1
        else:
            _poses = np.zeros((dataset_len, 72), dtype=float)
            _betas = np.zeros((dataset_len, 10), dtype=float)
            has_smpl = 0
